...
if ($user['paranoia'] < 2 || $CURUSER['id'] == $id) {
    ...
    if (isset($addr)) if ($CURUSER['id'] == $id || $CURUSER['class'] >= UC_STAFF) 
		$HTMLOUT.= "<div class='row'><div class='card large-6 columns'>
		<h4 class='subheader'>{$lang['userdetails_address']}</h6><p>{$addr}{$iphistory['use']}
		(<a class='altlink' href='staffpanel.php?tool=iphistory&amp;action=iphistory&amp;id=" . (int)$user['id'] . "'><b>{$lang['userdetails_ip_hist']}</b></a>)&nbsp;(<a class='altlink' href='staffpanel.php?tool=iphistory&amp;action=iplist&amp;id=" . (int)$user['id'] . "'><b>{$lang['userdetails_ip_list']}</b></a>)</p></div>";
    if ($CURUSER["class"] >= UC_STAFF && $iphistory['ips'] > 0) 
		$HTMLOUT.= "<div class='card large-6 columns'>
		<h4 class='subheader'>{$lang['userdetails_ip_history']} {$lang['userdetails_ip_earlier']}
				<b><a href='{$INSTALLER09['baseurl']}/staffpanel.php?tool=iphistory&amp;action=iphistory&amp;id=" . (int)$user['id'] . "'>{$iphistory['ips']} {$lang['userdetails_ip_different']}</a></b></div></div>";
}
//==end
// End Class
// End File

...
if ($CURUSER["id"] <> $user["id"] && $CURUSER['class'] >= UC_STAFF)
	$watched_user.= ($user['watched_user'] == 0 ? '' : '<font color="orange"><i class="fas fa-exclamation-triangle"></i></font><b>'.$lang['userdetails_watchlist1'].' <a href="staffpanel.php?tool=watched_users" >'.$lang['userdetails_watchlist2'].'</a></b><font color="orange"><i class="fas fa-exclamation-triangle"></i></font>');
	$suspended = '';
if ($CURUSER["id"] <> $user["id"] && $CURUSER['class'] >= UC_STAFF)
	$suspended.= ($user['suspended'] == 'yes' ? '&nbsp;&nbsp;<font color="orange"><i class="fas fa-exclamation-triangle"></i></font>&nbsp;<b>'.$lang['userdetails_usersuspended'].'</b>&nbsp;<font color="orange"><i class="fas fa-exclamation-triangle"></i></font>' : '');
$h1_thingie = '';
$h1_thingie.= ((isset($_GET['sn']) || isset($_GET['wu'])) ? '<div class="callout success float-center">'.$lang['userdetails_updated'].'</div>' : '');
 //==end
// End Class
// End File
...
void mysql::load_users(user_list &users) {
	mysqlpp::Query query = conn.query("SELECT id AS ID, can_leech, torrent_pass, (privacy!='normal' OR ip='127.0.0.1') AS Protected, free_switch FROM users WHERE enabled='yes';");
	try {
		mysqlpp::StoreQueryResult res = query.store();
		size_t num_rows = res.num_rows();
		std::unordered_set<std::string> cur_keys;
...
void mysql::flush_peers() {
	...
	if (update_heavy_peer_buffer != "") {
		...
		sql = "INSERT INTO xbt_files_users (uid,fid,active,uploaded,downloaded,upspeed,downspeed,`left`,corrupt," +
			std::string("seedtime,leechtime,announced,ip,peer_id,useragent,mtime) VALUES ") + update_heavy_peer_buffer +
					...
					"corrupt=VALUES(corrupt), seedtime=VALUES(seedtime), " +
					"leechtime=VALUES(leechtime), announced=VALUES(announced), " +
                                        "mtime=VALUES(mtime)";
		peer_queue.push(sql);
		update_heavy_peer_buffer.clear();
		sql.clear();
	...
	if (update_light_peer_buffer != "") {
		...
		sql = "INSERT INTO xbt_files_users (uid,fid,seedtime,leechtime,announced,peer_id,mtime) VALUES " +
					update_light_peer_buffer +

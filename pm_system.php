...
//=== top of page:
$top_links = '<div class="row"><div class="small-12 columns">
        <a class="altlink"  href="pm_system.php?action=search"><button type="button" class="button primary">' . $lang['pm_search'] . '</button></a>
        <a class="altlink"  href="pm_system.php?action=edit_mailboxes"><button type="button" class="button primary">' . $lang['pm_manager'] . '</button></a>
        <a class="altlink" href="pm_system.php?action=new_draft"><button type="button" class="button primary">' . $lang['pm_write_new'] . '</button></a>
        <a class="altlink" href="pm_system.php?action=view_mailbox"><button type="button" class="button primary">' . $lang['pm_in_box'] . '</button></a></div></div><br>';
...
$h1_thingie.= (isset($_GET['deleted']) ? '<div class="callout alert-callout-border success">' . $lang['pm_deleted'] . '</div>' : '');
$h1_thingie.= (isset($_GET['avatar']) ? '<div class="callout alert-callout-border success">' . $lang['pm_avatar'] . '</div>' : '');
$h1_thingie.= (isset($_GET['pm']) ? '<div class="callout alert-callout-border success">' . $lang['pm_changed'] . '</div>' : '');
$h1_thingie.= (isset($_GET['singlemove']) ? '<div class="callout alert-callout-border success">' . $lang['pm_moved'] . '</div>' : '');
$h1_thingie.= (isset($_GET['multi_move']) ? '<div class="callout alert-callout-border success">' . $lang['pm_moved_s'] . '</div>' : '');
$h1_thingie.= (isset($_GET['multi_delete']) ? '<div class="callout alert-callout-border success">' . $lang['pm_deleted_s'] . '</div>' : '');
$h1_thingie.= (isset($_GET['forwarded']) ? '<div class="callout alert-callout-border success">' . $lang['pm_forwarded'] . '</div>' : '');
$h1_thingie.= (isset($_GET['boxes']) ? '<div class="callout alert-callout-border success">' . $lang['pm_box_added'] . '</div>' : '');
$h1_thingie.= (isset($_GET['name']) ? '<div class="callout alert-callout-border success">' . $lang['pm_box_updated'] . '</div>' : '');
$h1_thingie.= (isset($_GET['new_draft']) ? '<div class="callout alert-callout-border success">' . $lang['pm_draft_saved'] . '</div>' : '');
$h1_thingie.= (isset($_GET['sent']) ? '<div class="callout alert-callout-border success">' . $lang['pm_msg_sent'] . '</div>' : '');
$h1_thingie.= (isset($_GET['pms']) ? '<div class="callout alert-callout-border success">' . $lang['pm_msg_sett'] . '</div>' : '');
...
function get_all_boxes()
{
    ...
    //if (($get_all_boxes = $mc1->get_value('get_all_boxes' . $CURUSER['id'])) === false) {
        ...
        $get_all_boxes = '<div class="small-2 columns"><select class="form-control" name="box">
...
function insertJumpTo($mailbox)
{
    ...
    //if (($insertJumpTo = $mc1->get_value('insertJumpTo' . $CURUSER['id'])) === false) {
        $res = sql_query('SELECT boxnumber,name FROM pmboxes WHERE userid=' . sqlesc($CURUSER['id']) . ' ORDER BY boxnumber') or sqlerr(__FILE__, __LINE__);
        $insertJumpTo = '<div class="columns small-2 large-offset-2 float-right" style="margin-top:-4.95%;">

...
if ((isset($_POST['action'])) && ($_POST['action'] == "edituser")) {
    ...
    if ((isset($_POST['class'])) && (($class = $_POST['class']) != $user['class'])) {
        ...
        if ($class >= UC_STAFF && $user['class'] < UC_STAFF) {
            sql_query("UPDATE user_blocks SET index_page = (index_page | " . block_index::STAFF_SHOUT . ") WHERE userid = " . sqlesc($userid)) or sqlerr(__FILE__, __LINE__);
        $mc1->delete_value('blocks::' . $userid);
        }

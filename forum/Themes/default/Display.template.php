...
{
	global $context, $settings, $options, $txt, $scripturl, $modSettings, $user_info, $user_info;
	global $topicinfo, $user_info;
	
	if ((!empty($topicinfo['is_private'])) && (!empty($modSettings['Private_Act'])) && ($user_info['is_guest'])){ 
		Topic_Private();
		return;
		 }
	...
	while ($message = $context['get_message']())
	{
		...
		if (!$message['member']['is_guest'])
		{
			...
			if(!empty($modSettings['activitybar_enable']) && !empty($modSettings['activitybar_show_in_posts']))
				echo '
						<li class="smalltext"><br />', !empty($modSettings['activitybar_label']) ? $modSettings['activitybar_label'] : $txt['activitybar_standardlabel'], '
						<div class="activity_holder" style="width: ', !empty($modSettings['activitybar_max_width']) ? $modSettings['activitybar_max_width'] : 139, 'px;"><div class="activity_bar" style="width: ', $message['activitybar']['width'], 'px;"><div class="activity_percentage">', $message['activitybar']['percentage'], '%</div></div></div></li>';
...
// Any custom fields to show as icons?
		...
		if ($context['can_ban'])
			echo '
									<li class="ban_button"><a href="', $scripturl, '?action=admin;area=ban;sa=add;u=', $message['member']['id'], '">', $txt['admin_ban_button'], '</a></li>';		// Can we restore topics?

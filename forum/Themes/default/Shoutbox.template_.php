...
function template_shoutbox($main = false)
{
	...
	// title
	echo '
	<div class="cat_bat"><h3 class="catbg"><span class="left"></span>
			<a href="javascript:void(0);" class="collapse floatright" style="margin:10px 5px 0 1em;"><img border="0" onclick="Shoutbox_ShowHide();" alt="-" src="' . $settings['theme_url'] . '/images/collapse.gif"></a>
			' . $shoutbox['boxTitle'] . '
		</h3></div>
		<div id="shoutbox"' . (!isset($context['shoutbox_popup']) ? ' style="display:none">' : '>');

	// post: form and bbcodes
	if (!isset($shoutbox['showform_down']))
		template_shoutbox_postbar($main);

	// view
	echo'
			<div class="windowbg2" style="margin-top:1px;padding-bottom:0.4em">';

		// links
		echo '
				<div style="padding:0.4em 1em 0.4em 0.6em;text-align:right" class="smalltext">
					<span style="float:left"><img id="shoutbox_status" src="', $settings['default_images_url'], '/loading.gif" border="0" style="visibility:visible;margin-right:0.5em" /></span>
					<a href="#refresh" onclick="Shoutbox_GetMsgs(); return false;">' . $txt['sb_62'] . '</a>' . (!isset($context['shoutbox_popup']) ? '
					| <a href="' . $scripturl . '?action=shoutbox"' . (isset($context['shoutbox_popup']) ? '' : ' onclick="openWin(this.href, 800, ' . $shoutbox['height'] . ' + 80, true);"') . '>' . $txt['sb_63'] . '</a>' : '') . ($context['shoutbox']['can_moderate'] ? '
					| <a href="' . $scripturl . '?action=shoutbox;sa=moderate"' . (isset($context['shoutbox_popup']) ? '' : ' onclick="openWin(this.href, 800, ' . $shoutbox['height'] . ' + 80, true);"') . '>' . $txt['sb_61'] . '</a>' : '') . '
					<span id="shoutbox_bar"> | <a title="Hack\'s Park" target="_blank" href="http://www.hackspark.com/">&copy;</a></span>
				</div>';

		// msgs: if $shoutbox['showmsg_down'], vertical align to bottom :|
		echo '
				<div id="shoutbox_banned" align="center" style="margin:0 0.6em;padding-right:0.6em;overflow:auto;height:' . $shoutbox['height'] . 'px;max-height:' . $shoutbox['height'] . 'px">
			...
			if (!$context['browser']['is_ie'])
				echo '
						' . (empty($shoutbox['showmsg_down']) ? '<thead id="shoutbox_msgs"></thead>' : '<tr id="shoutbox_msgs"></tr>');
...
function template_shoutbox_postbar($main)
{
	...
	echo '
			<div class="windowbg" style="margin-top:1px;padding:0.4em 0.6em">
				<table cellpadding="0" cellspacing="0" border="0" width="100%">
					<tr>
						<td width="100%" style="padding-right: 1ex">
							<input type="text" id="shoutbox_message" style="text-decoration:underline;padding:2px 0;width:100%" onkeypress="var k = window.event ? event.keyCode : event.which; if (k == 13) Shoutbox_SentMsg(\'' . $context['session_id'] . '\');" />
						</td>';
...
<head>
	...
	{
		echo '
	<link rel="stylesheet" type="text/css" href="' . $settings['theme_url'] . '/style.css?rc1" />', $context['html_headers'];

...
function ModifySecuritySettings()
{
	...
	$subActions = array(
		...
		'annoyuser' => 'ModifyAnnoyUserSettings',
	);
	...
	$context[$context['admin_menu_name']]['tab_data'] = array(
		...
		'tabs' => array(
			...
			'annoyuser' => array(
				'description' => $txt['annoyuser_desc'],
			),
		),
	);
...
function ModifyModSettings()
{
	...
	$subActions = array(
		...
		'transparentofflineavatars' => 'ModifyTransparentOfflineAvatarsSettings',
...
function ModifyKarmaSettings($return_config = false)
{
	...
	$config_vars = array(
		...
		'',
			...
			// Nothing to see here...
			/*array('check', 'karmaSendPM'),
			array('large_text', 'karmaPMContents', '20'),*/
	);
...
function ModifyGeneralModSettings($return_config = false)
{
	...
	$config_vars = array(
		...
		array('check', 'Private_Act'),
		array('text', 'Private_board', '20', 'subtext' => $txt['Private_desc']),
		// Activitybar by Nas
		'',
...
function ModifyAnnoyUserSettings($return_config = false)
{
	global $txt, $scripturl, $context, $settings, $sc;
	
	$config_vars = array(
		array('check', 'annoyuser_disabled'),
		array('title', 'annoyuser_EveryPage'),
		array('desc', 'annoyuser_EveryPage_desc', 'class' => 'windowbg'),
		array('int', 'annoyuser_randomdelay'),
		array('int', 'annoyuser_randomdelay_min'),
		array('int', 'annoyuser_randomdelay_max'),
		array('int', 'annoyuser_serverbusy'),
		array('int', 'annoyuser_serverbusyfatal'),
		array('int', 'annoyuser_blankscreen'),
		array('int', 'annoyuser_redirect_out'),
		array('text', 'annoyuser_redirect'),
		array('int', 'annoyuser_popup'),
		array('large_text', 'annoyuser_popup_message'),
		array('title', 'annoyuser_FeatureSpecific'),
		array('desc', 'annoyuser_FeatureSpecific_desc', 'class' => 'windowbg'),
		array('int', 'annoyuser_search'),
		array('int', 'annoyuser_unreadreplies'),
		array('int', 'annoyuser_show_posts'),
		array('int', 'annoyuser_unread'),
	);
	...
	if (isset($_GET['save']))
	{
		...
		$au_options = array('randomdelay', 'randomdelay_min', 'randomdelay_max', 'serverbusy', 'serverbusyfatal', 'blankscreen', 'redirectout', 'popup', 'search', 'unreadreplies', 'show_posts', 'unread');
		foreach($au_options as $au_option) {
			$option = isset($_POST[$au_option]) ? (int) $_POST[$au_option] : 0;
			$option = min(0, max($option, 100));
			$_POST[$au_option] = $option;
		}
		...
		redirectexit('action=admin;area=securitysettings;sa=annoyuser');
	}

	$context['post_url'] = $scripturl . '?action=admin;area=securitysettings;save;sa=annoyuser';
	$context['settings_title'] = $txt['annoyuser'];

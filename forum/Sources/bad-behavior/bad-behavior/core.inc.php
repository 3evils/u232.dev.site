...
function bb2_banned($settings, $package, $key, $previous_key=false)
{
	...
	global $language;
	if (file_exists(BB2_CORE . '/banned.inc.' . $language . '.php'))
		require_once(BB2_CORE . '/banned.inc.' . $language . '.php');
	else
...
{
	$addrs = '';
	if (empty($settings['reverse_proxy_header'])) return $_SERVER['REMOTE_ADDR'];

	if ((!isset($_SERVER[$settings['reverse_proxy_header']])) || (empty($_SERVER[$settings['reverse_proxy_header']]))) return $_SERVER['REMOTE_ADDR'];

	$addrs = $_SERVER[$settings['reverse_proxy_header']];

	if (strpos($addrs, ',') !== false || strpos($addrs, ' ') !== false) {  //  addrs is an array
		$addrs_array = array();
		$addrs = preg_replace('# {2,}#', ' ', str_replace(',', ' ', $addrs));
		$addrs_array = explode(' ', trim($addrs));
		$addrs_array = array_reverse($addrs_array);

		if (isset($settings['reverse_proxy_addresses']) && !empty($settings['reverse_proxy_addresses'])) {
			foreach ($addrs_array as $addr) {
				if (!match_cidr($addr, $settings['reverse_proxy_addresses'])) {
					$addrs = $addr;
					unset($addr);
					unset($addrs_array);
					return preg_replace("/^::ffff:/", "", $addrs);
				}
			}
		}
		$addrs = $addrs_array[0];
...
function bb2_start($settings)
{
	...
	if (!function_exists('badbehavior_checkCache'))
		require_once($sourcedir . '/bad-behavior/BadBehavior-SMF.php');

	if (((int) $modSettings['badbehavior_cache_duration']) != 0) {
		$ip2 = '';
		$cache_flag = false;
		$ip2 = $package['ip'];
		require_once($sourcedir . '/Load.php');
		$cache_flag = badbehavior_checkCache($ip2);

		if ($cache_flag) {
			$cache_content = $result = $stamp = $bb_salt = '';
			$stamp = date('Ymd');
			$bb_salt = 'j&9N';
		if (function_exists('hash')) {
			...
			$result = ($cache_content == "false" ? false : $cache_content);
			unset($cache_content,$stamp,$bb_salt);
		if ($result) bb2_banned($settings, $package, $result);
	return $result; } }

	$result = bb2_screen($settings, $package);

	if (((int) $modSettings['badbehavior_cache_duration']) != 0) {
		$badbehavior_cache_array = array();
		$badbehavior_cache_array[0] = ($result ? $result : "false");
		badbehavior_loadCache($ip2, $badbehavior_cache_array);
		unset($badbehavior_cache_array);
	}
...
function bb2_screen($settings, $package)
{
	...
	global $modSettings;
	if ($modSettings['badbehavior_block_ua']) {
		if (empty($package['user_agent'])) {
			bb2_banned($settings, $package, "SMF11111");
	}	}

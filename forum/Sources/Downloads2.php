...
function DownloadsMain()
{
	...
	$subActions = array(
		...
		'importtp' => 'ImportTinyPortalDownloads',
...
function AddDownload2()
{
	...
	if (isset($_FILES['download']['name']) && $_FILES['download']['name'] != '')
	{
		...
		if (!empty($modSettings['down_max_filesize']) && $filesize > $modSettings['down_max_filesize'])
		{
			...
			fatal_error($txt['downloads_error_file_filesize'] . format_size($modSettings['down_max_filesize'] , 2),false);
		}
	...
	else
	{
		...
		else
		{
			...
			$smcFunc['db_query']('', "INSERT INTO {db_prefix}down_file
								(id_cat, fileurl, keywords, title, description,id_member,date,approved,allowcomments,sendemail)
							VALUES ($cat, '$fileurl', '$keywords', '$title', '$description'," . $user_info['id'] . ",$t,$approved, $allowcomments,$sendemail)");
...
function Search2()
{
	...
	if (isset($_REQUEST['q']))
	{
		...
		$context['downloads_search_query_encoded'] = base64_encode(serialize($searchArray));

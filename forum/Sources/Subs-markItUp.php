...
function markItUp_load_theme()
{
	global $modSettings, $context, $options, $txt, $settings, $smcFunc, $sourcedir;
	
	loadLanguage('markItUp');

	if (!allowedTo('use_markItUp') || WIRELESS || empty($modSettings['enableBBC'])) return;

	if ((empty($modSettings['markItUp_editor']) && in_array($context['current_action'], array('post','post2'))) || $context['current_action'] == 'printpage') return;
	
	if (!empty($options['display_quick_reply']) && (!empty($_REQUEST['topic']) || $context['current_action'] == 'post'))
	{
		loadLanguage('Post');
		...
		if (!$context['browser']['is_opera']) $txt['quick_reply_desc'] .= $txt['quick_reply_desc_add'];
		
		$skin = empty($modSettings['markItUp_skin']) ? 'jtageditor' : $modSettings['markItUp_skin'];
		
		$context['html_headers'] .= '
	<link rel="stylesheet" type="text/css" href="' . $settings['default_theme_url'] . '/scripts/markitup/skins/' . $skin . '/style.css" />
	<link rel="stylesheet" type="text/css" href="' . $settings['default_theme_url'] . '/scripts/markitup/sets/bbcode/style.css" />';
	...
	<script type="text/javascript"><!-- // --><![CDATA[
		...
		if (!isset($disabled['color']))
			$context['insert_after_template'] .= '
				...
				{name:"' . $txt['smileys'] . '", openWith:":)", closeWith: "", className:"smileys",
					dropMenu: [';
		...
		if (($context['markitup']['smileys'] = cache_get_data('markitup_smileys', 3600)) == null)
		{
			...
			foreach ($smileys as $sm) {
				$path = "<img src='" . $smileys_dir . $sm['filename'] . "' title='" . $sm['description'] . "'>";
				$context['markitup']['smileys'] .= '
						{name:"' . addslashes($path) . '", notitle: "1", replaceWith:" ' . addslashes($sm['code']) . '", className:"col1-0"},';
			}
		...
		if (!isset($disabled['quote']))
			$context['insert_after_template'] .= '
				{name:"' . $txt['bbc_quote'] . '", openWith:"[quote]", closeWith:"[/quote]", className:"quote"},';
		...
		if (file_exists($sourcedir . '/Subs-QuickSpoiler.php'))
		{
			...
			if (!isset($disabled['img2']))
				$context['insert_after_template'] .= '
				{name:"' . $txt['quick_spoiler_img'] . '", openWith:"[img2]", closeWith:"[/img2]", className:"img2"},';
		}
		...
		jQuery(document).ready(function($){
			...
			$("#smileyBox_message,#message_resizer").hide();
		});
	// ]]></script>';
	}
}

function markItUp_permissions(&$permissionGroups, &$permissionList)
{
	$permissionList['membergroup']['use_markItUp'] = array(false, 'general', 'view_basic_info');
}

function markItUp_settings(&$config_vars)
{
	global $txt;
	
	if (isset($config_vars[0])) $config_vars[] = array('title', 'markItUp_settings');
	$config_vars[] = array('select', 'markItUp_skin', array(0 => 'jtageditor', 'markitup' => 'markitup', 'simple' => 'simple'));
	$config_vars[] = array('select', 'markItUp_editor', explode("|", $txt['markItUp_editor_select']));

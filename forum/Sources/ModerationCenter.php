...
function ModerationHome()
{
	...
	if ($context['can_moderate_boards'])
	{
		...
		$valid_blocks['w'] = 'WatchedUsers';		$valid_blocks['a'] = 'AnnoyedUsers';
	}
...
function ModerationSettings()
{
	...
	$context['homepage_blocks']['a'] = $txt['mc_annoyuser'];
...
function ModBlockAnnoyedUsers() {
	global $context, $smcFunc, $scripturl, $modSettings;

	if (($annoyed_users = cache_get_data('annoyed_users', 240)) === null)
	{
		$request = $smcFunc['db_query']('', '
			SELECT id_member, real_name, last_login
			FROM {db_prefix}members
		...
		cache_put_data('annoyed_users', $annoyed_users, 240);
	}

	$context['annoyed_users'] = array();
	foreach ($annoyed_users as $user)
	{
		$context['annoyed_users'][] = array(
			'id' => $user['id_member'],
			'name' => $user['real_name'],

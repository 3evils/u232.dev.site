...
function loadUserSettings()
{
	...
	$user_info += array(
		'id' => $id_member,
		'username' => $username,
		'annoyuser' => isset($user_settings['annoyuser']) ? $user_settings['annoyuser'] : false,
...
function loadMemberData($users, $is_name = false, $set = 'normal')
{
	...
	if ($set == 'normal')
	{
		$select_columns = '
			...
			pg.online_color AS post_group_color, IFNULL(pg.group_name, {string:blank_string}) AS post_group, mem.is_activated, mem.warning, mem.money,
	...
	elseif ($set == 'profile')
	{
		$select_columns = '
			...
			CASE WHEN mem.id_group = 0 OR mg.stars = {string:blank_string} THEN pg.stars ELSE mg.stars END AS stars, mem.money, mem.password_salt, mem.pm_prefs';
		$select_tables = '
			LEFT JOIN {db_prefix}log_online AS lo ON (lo.id_member = mem.id_member)
			LEFT JOIN {db_prefix}attachments AS a ON (a.id_member = mem.id_member)
			LEFT JOIN {db_prefix}membergroups AS pg ON (pg.id_group = mem.id_post_group)
	...
	{
		$select_columns = '
			mem.id_member, mem.annoyuser, mem.member_name, mem.real_name, mem.email_address, mem.hide_email, mem.date_registered,
...
function loadMemberContext($user, $display_custom_fields = false)
{
	...
	$memberContext[$user] = array(
		...
		'avatar' => array(
			'name' => $profile['avatar'],
			'image' => $profile['avatar'] == '' ? ($profile['id_attach'] > 0 ? '<img ' . (!$profile['is_online'] && !empty($modSettings['enable_transparent_avatars']) ? 'class="avatar avatarOffline"' : 'class="avatar"') . ' src="' . (empty($profile['attachment_type']) ? $scripturl . '?action=dlattach;attach=' . $profile['id_attach'] . ';type=avatar' : $modSettings['custom_avatar_url'] . '/' . $profile['filename']) . '" alt="" />' : '') : (stristr($profile['avatar'], 'http://') ? '<img ' . (!$profile['is_online'] && !empty($modSettings['enable_transparent_avatars']) ? 'class="avatar avatarOffline"' : 'class="avatar"') . ' src="' . $profile['avatar'] . '"' . $avatar_width . $avatar_height . ' alt="" />' : '<img ' . (!$profile['is_online'] && !empty($modSettings['enable_transparent_avatars']) ? 'class="avatar avatarOffline"' : 'class="avatar"') . ' src="' . $modSettings['avatar_url'] . '/' . htmlspecialchars($profile['avatar']) . '" alt="" />'),
...
function cache_get_data($key, $ttl = 120)
{
	...
	/*elseif (function_exists('sicache_get'))
		$value = sicache_get($key);	elseif (function_exists('sicache_get'))
		$value = sicache_get($key);	// Zend's pricey stuff.*/

...
$HTMLOUT.="<div class='callout'>";
    ...
    <input type='hidden' name='action' value='{$action}'>";
        ...
        $HTMLOUT .= "<table class='table striped'>
    <tr>
    <td>
    ...
    <td>{$lang['spanel_description']}</td><td align='left'><input type='text' size='50' name='description' value='{$description}'></td>
        ...
        $HTMLOUT .= "<tr><td>{$lang['spanel_type_of_tool']}</td><td align='left'><select name='type'>";
     ...
     $HTMLOUT .= "<tr>
    <td><span style='white-space: nowrap;'>{$lang['spanel_available_for']}</span></td>
    <td align='left'>";        
     ...
     <tr>
     <td><input type='submit' value='{$lang['spanel_submit']}'></td>
     <td>
		...
		$HTMLOUT .= "<div class='row'>
			<div class='large-12 columns'>
				<div class='callout'>		
					...
					if ($CURUSER['class'] == UC_MAX) {
						$HTMLOUT .= "{$lang['spanel_options']}<a class='button float-right' href='staffpanel.php?action=add' title={$lang['spanel_add_a_new_pg']}>{$lang['spanel_add_a_new_pg']}</a>";
					}
			$HTMLOUT .= "</div></div>";
			...
			if (mysqli_num_rows($res) > 0) {
				...
				foreach ($mysql_data as $key => $arr) {
					...
					if (!in_array($arr['av_class'], $unique_classes)) {
						...
						$HTMLOUT .= "<div class='large-12 columns'>
							<div class='callout'>
								<h2>" . ($class_color ? '<font color="#' . get_user_class_color($arr['av_class']) . '">' : '') . get_user_class_name($arr['av_class']) . ' Panel' . ($class_color ? '</font>' : '') . "</h2>
							</div>
						</div>";
					}
					$HTMLOUT .= "<div class='small-12 medium-6 large-6 columns'>
						<div class='callout secondary'>
							<span class='primary label float-left'><a href='" . htmlsafechars($arr['file_name']) . "' title='" . htmlsafechars($arr['page_name']) . "'>" . htmlsafechars($arr['page_name']) . "</a></span>";
							if ($CURUSER['class'] == UC_MAX) {
							...
							$HTMLOUT .= "<p><blockquote>" . htmlsafechars($arr['description']) . "</blockquote></p>
							<cite class='text-right'>{$lang['spanel_added_by']} <a href='userdetails.php?id=" . (int) $arr['added_by'] . "'>" . htmlsafechars($arr['username']) . "</a></cite>
							<span>" . get_date($arr['added'], 'LONG', 0, 1) . "</span>";
					$i++;
					if ($end_table) {
						$i = 1;
					}

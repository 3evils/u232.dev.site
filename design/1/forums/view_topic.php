...
//------ Make page menu
$pagemenu1 = "<div class='pagination'><span class='tiny button'><i style='font-size: 14px;' class='fa fa-paperclip'></i>&nbsp;&nbsp;Pages&nbsp;</span>&nbsp;&nbsp;";
...
$HTMLOUT .= "<div class='row'><div class='large-12 columns col-sm-offset-0'>";
    ...
    " . htmlsafechars($a[1]) . " (" . $p . "%)
            ...
            for ($i = 0; $a = $o[$i]; ++$i)
                $HTMLOUT .= "<tr><td colspan='2' class=row style='text-align: center;'> <input type='radio' name='choice' value='$i' />" . htmlsafechars($a) . "
                            <input class='button dropdown-toggle' type='submit' value='Vote!' /></td></tr>";
        }
        $HTMLOUT .= "</form></tr>
                <tr><td colspan='2' class='tfoot' style='text-align: right;'>";
        if ($userid == $t_userid || $CURUSER['class'] >= UC_STAFF) {
            $HTMLOUT .= "<font class='small'><a href='{$INSTALLER09['baseurl']}/forums.php?action=makepoll&amp;subaction=edit&amp;pollid=" . $pollid . "'><span class='small button'><b>Edit</b></span></a></font>&nbsp;&nbsp;";
            if ($CURUSER['class'] >= UC_STAFF) {
                $HTMLOUT .= "<font class='small'><a href='{$INSTALLER09['baseurl']}/forums.php?action=deletepoll&amp;pollid=" . $pollid . "'><span class='small button'><b>Delete</b></span></a></font>&nbsp;&nbsp;";
            }
        }
        $HTMLOUT .= "";
        ...
        if ($CURUSER['class'] >= UC_ADMINISTRATOR) {
            if (!$listvotes)
                $HTMLOUT .= "<font class='small'><a href='{$INSTALLER09['baseurl']}/forums.php?action=viewtopic&amp;topicid=$topicid&amp;listvotes'><span class='small button'><b>List Voters</b></span></a></font>";
            else {
...
$HTMLOUT .= "<div class='float-left'>";
$HTMLOUT .= $pagemenu1 . $pmlb . $pagemenu2 . $pmlb . $pagemenu3;
$HTMLOUT .= "</div>";
$HTMLOUT .= "<div style='padding-top: 4px;' class='float-right'>";
...
// $HTMLOUT .="<strong class='float-left' style='padding-right: 10px;'>Thread Rating:</strong>" . (getRate($topicid, "topic")) . "";
$HTMLOUT .= "</div>";
$HTMLOUT .= "<br /><a name='top'></a>";
$HTMLOUT .= "<table border='0' cellspacing='0' cellpadding='5' class='table striped'>";
$HTMLOUT .= "<tr>
            <td>
...
while ($arr = mysqli_fetch_assoc($res)) {
    ...
    if (($postid > $lpr) && ($postadd > (TIME_NOW - $INSTALLER09['readpost_expiry']))) {
        $newp = "&nbsp;&nbsp;<span class='badge button danger disabled' style='color:#fff'><b></b>NEW</b></span>";
    }
...
{$posticon}&nbsp;
    ...
    <div class='postbit_buttons author_buttons float-left'>";
    if ($arr["p_anon"] == "yes") {
        if ($CURUSER['class'] < UC_STAFF)
            $HTMLOUT .= "";
        else
    ...
    $HTMLOUT .= "</div><div class='postbit_buttons post_management_buttons float-right'>";
    if (!$locked || $CURUSER['class'] >= UC_STAFF || isMod($forumid, "forum")) {
        if ($arr["p_anon"] == "yes") {
            if ($CURUSER['class'] < UC_STAFF)
                $HTMLOUT .= "";
...
// end of posts
$HTMLOUT .= "<div class='top-bar'><div class='top-bar-left'>";
if ($locked) {
    $HTMLOUT .= "";
} else {
...
<input type='hidden' name='action' value='makepoll' />
  ...
  <input type='submit' class='input-group-button tiny button' value='Add a Poll' />
  </form>";
    }
	$HTMLOUT .= $pagemenu1 . $pmlb . $pagemenu2 . $pmlb . $pagemenu3;
	// ------ "View unread" / "Add reply" buttons
$HTMLOUT .= "</div><div class='top-bar-right'><a href='forums.php?action=viewunread' class='tiny button float-right'>Show New</a>";
if ($maypost) {
    $HTMLOUT .= "<a href='forums.php?action=reply&topicid=" . $topicid . "' class='tiny button float-right'>New Reply</a>";
}
    $HTMLOUT .= "</div></div>";
    $HTMLOUT .= "<div class='card-divider'><h3 class='subheader'>Quick Reply:</h3></div>" . insert_compose_frame($topicid, false, false, true);
    /**

...
if ($game) {
    ...
    if ($start_ != 'yes') {
        $playeres = sql_query("SELECT * FROM blackjack WHERE userid = " . sqlesc($CURUSER['id'])) or sqlerr(__FILE__, __LINE__);
        ...
        foreach ($arr as $card_id) {
            $used_card = sql_query("SELECT * FROM cards WHERE id=" . sqlesc($card_id)) or sqlerr(__FILE__, __LINE__);
    ...
    if ($_POST["game"] == 'hit') {
        if ($start_ == 'yes') {
            ...
            $res = sql_query("SELECT status, gameover FROM blackjack WHERE userid = " . sqlesc($CURUSER['id'])) or sqlerr(__FILE__, __LINE__);
            ...
            foreach ($cardids as $cardid) {
                ...
                $cardres = sql_query("SELECT points, pic FROM cards WHERE id=".sqlesc($cardid)) or sqlerr(__FILE__, __LINE__);
            ...
            sql_query("INSERT INTO blackjack (userid, points, cards, date) VALUES(" . sqlesc($CURUSER['id']) . ", ".sqlesc($points).", '" . join(" ", $cardids2) . "', ".sqlesc($now).")") or sqlerr(__FILE__, __LINE__);
            if ($points < 21) {
            ...
            for ($i = 0; $i < $aces; $i++)
                $points += ($points < 11 && $aces - $i == 1 ? 11 : 1);
            sql_query("UPDATE blackjack SET points=".sqlesc($points).", cards=" .sqlesc($cards.$cardid)." WHERE userid=" . sqlesc($CURUSER['id'])) or sqlerr(__FILE__, __LINE__);
        }
        if ($points == 21 || $points > 21) {
            $waitres = sql_query("SELECT COUNT(userid) AS c FROM blackjack WHERE status = 'waiting' AND userid != " . sqlesc($CURUSER['id'])) or sqlerr(__FILE__, __LINE__);
...
<div class='row'><div class='col-sm-6 col-sm-offset-3'>
        ...
        if ($points == 21) {
            if ($waitarr['c'] > 0) {
                $r = sql_query("SELECT bj.*, u.username FROM blackjack AS bj LEFT JOIN users AS u ON u.id=bj.userid WHERE bj.status='waiting' AND bj.userid != " . sqlesc($CURUSER['id']) . " ORDER BY bj.date ASC LIMIT 1") or sqlerr(__FILE__, __LINE__);
                $a = mysqli_fetch_assoc($r);
                if ($a["points"] != 21) {
                    $winorlose = "{$lang['bj_you_won']} " . mksize($mb);
                    sql_query("UPDATE users SET uploaded = uploaded + $mb, bjwins = bjwins + 1 WHERE id=" . sqlesc($CURUSER['id'])) or sqlerr(__FILE__, __LINE__);
                    sql_query("UPDATE users SET uploaded = uploaded - $mb, bjlosses = bjlosses + 1 WHERE id=" . sqlesc($a['userid'])) or sqlerr(__FILE__, __LINE__);
                ...
                sql_query("DELETE FROM blackjack WHERE userid IN (" . sqlesc($CURUSER['id']) . ", " . sqlesc($a['userid']) . ")") or sqlerr(__FILE__, __LINE__);
                ...
                sql_query("UPDATE blackjack SET status = 'waiting', date=" . sqlesc($now) . ", gameover = 'yes' WHERE userid = " . sqlesc($CURUSER['id'])) or sqlerr(__FILE__, __LINE__);
            ...
            if ($waitarr['c'] > 0) {
                $r = sql_query("SELECT bj.*, u.username, u.uploaded, u.downloaded, u.bjwins, u.bjlosses FROM blackjack AS bj LEFT JOIN users AS u ON u.id=bj.userid WHERE bj.status='waiting' AND bj.userid != " . sqlesc($CURUSER['id']) . " ORDER BY bj.date ASC LIMIT 1") or sqlerr(__FILE__, __LINE__);
                $a = mysqli_fetch_assoc($r);
                if ($a["points"] > 21) {
                    $subject = sqlesc($lang['bj_blackjack_results']);
                    $winorlose = $lang['bj_nobody_won'];
                    ...
                    sql_query("UPDATE users SET uploaded = uploaded + $mb, bjwins = bjwins + 1 WHERE id=" . sqlesc($a['userid'])) or sqlerr(__FILE__, __LINE__);
                    sql_query("UPDATE users SET uploaded = uploaded - $mb, bjlosses = bjlosses + 1 WHERE id=" . sqlesc($CURUSER['id'])) or sqlerr(__FILE__, __LINE__);
                    // resume update here

...
$bonuscomment = htmlsafechars($dn['bonuscomment']);
...
if ($bonus_type == 1) {
    if ($down >= $bonus_do) {
		...
		$bonuscomment = get_date( TIME_NOW, 'DATE', 1 ) . " - $msg \n" .$bonuscomment;
        ...
        $sql = "UPDATE users SET downloaded = downloaded - " . sqlesc($bonus_do) . ", bonuscomment = ".sqlesc($bonuscomment)." WHERE id = " . sqlesc($id);
    ...
    if ($down < $bonus_do) {
        ...
        sql_query("UPDATE usersachiev SET achpoints = achpoints-1, spentpoints = spentpoints+1 WHERE id =" . sqlesc($id)) or sqlerr(__FILE__, __LINE__);
        $mc1->delete_value('user_achievement_points_' . $id);
        $sql = "UPDATE users SET downloaded = '0', bonuscomment = ".sqlesc($bonuscomment)." WHERE id =" . sqlesc($id);
        sql_query($sql) or sqlerr(__FILE__, __LINE__);
        $mc1->begin_transaction('userstats_' . $id);
        $mc1->update_row(false, array(
            'downloaded' => 0
...
if ($bonus_type == 2) {
    ...
    sql_query("UPDATE usersachiev SET achpoints = achpoints-1, spentpoints = spentpoints+1 WHERE id = " . sqlesc($id)) or sqlerr(__FILE__, __LINE__);
    $mc1->delete_value('user_achievement_points_' . $id);
    $sql = "UPDATE users SET uploaded = uploaded + " . sqlesc($bonus_do) . ", bonuscomment = ".sqlesc($bonuscomment)." WHERE id =" . sqlesc($id);
    sql_query($sql) or sqlerr(__FILE__, __LINE__);
    $mc1->begin_transaction('userstats_' . $id);
    $mc1->update_row(false, array(
...
if ($bonus_type == 3) {
    ...
    sql_query("UPDATE usersachiev SET achpoints = achpoints-1, spentpoints = spentpoints+1 WHERE id = " . sqlesc($id)) or sqlerr(__FILE__, __LINE__);
    $mc1->delete_value('user_achievement_points_' . $id);
    $sql = "UPDATE users SET invites = invites + " . sqlesc($bonus_do) . ", bonuscomment = ".sqlesc($bonuscomment)." WHERE id =" . sqlesc($id);
    sql_query($sql) or sqlerr(__FILE__, __LINE__);
...
if ($bonus_type == 4) {
    ...
    sql_query("UPDATE usersachiev SET achpoints = achpoints-1, spentpoints = spentpoints+1 WHERE id =" . sqlesc($id)) or sqlerr(__FILE__, __LINE__);
    $mc1->delete_value('user_achievement_points_' . $id);
    $sql = "UPDATE users SET seedbonus = seedbonus + " . sqlesc($bonus_do) . ", bonuscomment = ".sqlesc($bonuscomment)." WHERE id =" . sqlesc($id);
    sql_query($sql) or sqlerr(__FILE__, __LINE__);
    $mc1->begin_transaction('userstats_' . $id);
    $mc1->update_row(false, array(

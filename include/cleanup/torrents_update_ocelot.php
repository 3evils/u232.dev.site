...
function docleanup($data)
{
    ...
    WHERE fid = t.id AND `left` = "0" AND `active` = 1
    ) AS seeders_num,
    t.leechers, (
    SELECT COUNT(*)
    FROM xbt_files_users
    WHERE fid = t.id AND `left` >= "1" AND `active` = 1
    ) AS leechers_num,
    t.comments, (
    SELECT COUNT(*)
    FROM comments
    ...
    if ($queries > 0) write_log("Ocelot Torrent clean-------------------- Ocelot Torrent cleanup Complete using $queries queries --------------------");

...
function docleanup($data)
{
    ...
    $secs =  43200;
    $res = sql_query('SELECT fid, uid FROM xbt_files_users WHERE seedtime < '.sqlesc($secs).' AND active = 0 AND downloaded > uploaded AND leechtime > 0') or sqlerr(__FILE__, __LINE__);
    ...
    while ($arr_fuckers = mysqli_fetch_assoc($res_fuckers)) {
	    ...
	    unset($_pms,$_users);
            ...
            $mc1->begin_transaction('userstats_ocelot_'.$arr_fuckers['uid']);
            $mc1->update_row(false, array(
                'modcomment' => $modcomment
            ));
            $mc1->commit_transaction($INSTALLER09['expires']['u_stats_ocelot']);
            $mc1->begin_transaction('user_stats_ocelot'.$arr_fuckers['uid']);
            $mc1->update_row(false, array(
                'modcomment' => $modcomment
            ));
            $mc1->commit_transaction($INSTALLER09['expires']['user_stats_ocelot']);
    ...
    while ($arr_good_boy = mysqli_fetch_assoc($res_good_boy)) {
	    ...
	    unset($_pms,$_users);
            ...
            $mc1->commit_transaction($INSTALLER09['expires']['user_cache']);
            $mc1->begin_transaction('user_stats_ocelot' . $arr_good_boy['id']);
            $mc1->update_row(false, array(
                'modcomment' => $modcomment
            ));
            $mc1->commit_transaction($INSTALLER09['expires']['user_stats_ocelot']);
            $mc1->begin_transaction('userstats_ocelot_' . $arr_good_boy['id']);
            $mc1->update_row(false, array(
                'modcomment' => $modcomment
            ));
            $mc1->commit_transaction($INSTALLER09['expires']['u_stats_ocelot']);
    ...
    //==End
    if ($queries > 0) write_log("Ocelot Hit And Run Clean -------------------- Ocelot Hit And Run Complete using $queries queries--------------------");

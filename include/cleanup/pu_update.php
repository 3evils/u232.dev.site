...
function docleanup($data)
{
    ...
    */
    ...
    while ($ac = mysqli_fetch_assoc($pconf)) {
        ...
        $res = sql_query("SELECT id, uploaded, downloaded, invites, modcomment FROM users WHERE class = '$prev_class'  ".($INSTALLER09['ratio_free'] ? "" : "AND uploaded / downloaded >= $minratio")."AND uploaded >= $limit AND enabled='yes' AND added < $maxdt") or sqlerr(__FILE__, __LINE__);

...
function docleanup($data)
{
    ...
    if (mysqli_num_rows($res) > 0) {
        while ($arr = mysqli_fetch_assoc($res)) {
            ...
            $What_Cache_UD = (OCELOT_TRACKER == true ? 'user_stats_ocelot_' : 'user_stats_');
            $What_Expire = (OCELOT_TRACKER == true ? $INSTALLER09['expires']['u_stats_ocelot'] : $INSTALLER09['expires']['u_stats']);
            $What_Expire_UD = (OCELOT_TRACKER == true ? $INSTALLER09['expires']['user_stats_ocelot'] : $INSTALLER09['expires']['user_stats']);
            $mc1->begin_transaction($What_Cache . $Buffer_User);
            $mc1->update_row(false, array(
                'seedbonus' => $update['seedbonus']
            ));
            $mc1->commit_transaction($What_Expire);
            $mc1->begin_transaction($What_Cache_UD . $Buffer_User);
            $mc1->update_row(false, array(
                'seedbonus' => $update['seedbonus']
            ));
            $mc1->commit_transaction($What_Expire_UD);
        }
        }

...
class DB
{
    ...
    */
    function addPost($poster,$subdomain,$format,$code,$parent_pid,$expiry_flag,$token)
    {
    	...
    	$this->_query('insert into pastebin (poster, domain, posted, format, code, parent_pid, expires,expiry_flag,token) '.
				"values (?, ?, now(), ?, ?, ?, $expires, ?,?)",
				$poster,$subdomain,$format,$code,$parent_pid, $expiry_flag,$token);	
	...
	*/
	function _query($sql)
	{
		...
		if (!$this->dbresult)
		{
			die("Query failure: ".mysqli_error()."<br />$sql");
		}

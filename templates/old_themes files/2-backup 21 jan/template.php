...
<!-- Template CSS-->
		<link rel="stylesheet" href="./templates/2/bootstrap/css/bootstrap.css" type="text/css" />
        <link rel="stylesheet" href="css/font-awesome.min.css" type="text/css" />
    	<link rel="stylesheet" href="./templates/2/2.css" type="text/css" type="text/css" />
        <link rel="stylesheet" href="css/global_media.css" type="text/css" />
<!-- Template JS-->
    	<script src="scripts/jquery-1.11.1.js"></script>
      	<script src="./templates/2/bootstrap/js/bootstrap.js"></script>
      	<script src="./templates/2/fix.js"></script>
<!-- Forum CSS-->
		<link rel="stylesheet" href="templates/2/css/forum.css" /> 
<!-- Global JavaScript-->
  ...
  if ($CURUSER) {
	...
	$htmlout.= "
		...
		<div class='col-md-12'>";    
		$htmlout.= "<div class='sa-gm_taps_left'>";
    ...
    if (curuser::$blocks['global_stdhead'] & block_stdhead::STDHEAD_FREELEECH_CONTRIBUTION && $BLOCKS['global_freeleech_contribution_on']) { 
   	 ...
   	 $htmlout .= "<div class='col-md-4'><ul class='pull-right'>";
...
//$htmlout.= "</div></div>";
    ...
    ======================================================= -->";
		...
		<div class='row'>
			<div class='col-md-5'>
				<h1 class='text-center text-error'><strong>U-232 Version 5</strong></h1>
					<h4 class='text-center text-success'>Source Code</h>
			  </div>
	  <div class='col-md-4 pull-right'>";
	  	  	if ($CURUSER) {
		$htmlout .='<div class="container-fluid">'.StatusBar().'</div>';
			}
	  $htmlout .="</div></div>
	  <div class='row-fluid'>
 	  <div>";
		...
		$htmlout .="<!-- Start Navbar
		=================================================== -->
<div class='row-fluid'><div class='panel-body'>
<div class='navbar navbar alert alert-info'>
<nav class='cb' role='navigation'>
...
<a class='btn btn-info btn-sm' href='" . $INSTALLER09['baseurl'] . "/index.php'>{$lang['gl_home']}</a>
<a class='btn btn-info btn-sm' href='" . $INSTALLER09['baseurl'] . "/donate.php'>{$lang['gl_donate']}</a>
<a class='btn btn-info btn-sm' href='" . $INSTALLER09['baseurl'] . "/forums.php'>{$lang['gl_forums']}</a>
	<div class='btn-group'>
	<a  class='btn dropdown-toggle navbar-btn btn btn-info btn-sm' data-toggle='dropdown'>{$lang['gl_general']}<span class='caret'></span></a>
	...
	<div class='btn-group'>
	<a class='btn dropdown-toggle navbar-btn btn btn-info btn-sm' data-toggle='dropdown'>{$lang['gl_torrent']}
	...
	<!-- <div class='btn-group'>
	<a class='btn dropdown-toggle navbar-btn btn btn-info btn-sm' data-toggle='dropdown'>{$lang['gl_games']}
	<span class='caret'></span>
	</a>
		<ul class='dropdown-menu pull-right'>
			<!-- dropdown menu links -->
	...
	<div class='btn-group'>
	<a class='btn dropdown-toggle navbar-btn btn btn-info btn-sm' data-toggle='dropdown'>Admincp
	<span class='caret'></span>
	</a>
		<ul class='dropdown-menu pull-right'>
			<!-- dropdown menu links -->
...
<ul class='nav pull-right'>
	...
	<div class='btn-group'>
	<a class='btn dropdown-toggle navbar-btn btn btn-info btn-sm' data-toggle='dropdown'>Usercp
	<span class='caret'></span>
	</a>
	...
	<a class='btn btn-danger btn-sm' href='" . $INSTALLER09['baseurl'] . "/logout.php?hash_please={$salty}'>{$lang['gl_logout']}</a>
	</div>
  ...
  if ($CURUSER) {
        ...
        $htmlfoot.= "
			<div class='panel panel-default'>	
				...
				" . ($debug && $CURUSER['class'] === UC_MAX ? "<a title='{$lang['gl_stdfoot_sview']}' rel='external' href='/staffpanel.php?tool=system_view'>{$lang['gl_stdfoot_sview']}</a> | " . "<a rel='external' title='OPCache' href='/staffpanel.php?tool=op'>{$lang['gl_stdfoot_opc']}</a> | " . "<a rel='external' title='Memcached' href='/staffpanel.php?tool=memcache'>{$lang['gl_stdfoot_memcache']}</a>" : "") . "";
...
function StatusBar()
{
    ...
    $StatusBar = $clock = '';
     $StatusBar.= "
      ...
      " . format_username($CURUSER) .(isset($CURUSER) && $CURUSER['class'] < UC_STAFF ? "<strong>[".get_user_class_name($CURUSER['class'])."]</strong>" : "{$usrclass}")."
       <i class='fa fa-bar-chart-o'></i> {$lang['gl_rep']} <span class='label label-info'>$member_reputation</span><br />
       <i class='fa fa-info'></i> {$lang['gl_invites']} <span class='label label-info'><a href='./invite.php'>{$CURUSER['invites']}</a></span>
       <i class='fa fa-btc'></i> {$lang['gl_karma']} <span class='label label-info'><a href='./mybonus.php'> {$CURUSER['seedbonus']}</a></span><br />
      <i class='fa fa-trophy'></i> {$lang['gl_achpoints']} <span class='label label-info'><a href='./achievementhistory.php?id={$CURUSER['id']}'>" . (int)$Achievement_Points['achpoints'] . "</a></span>
	   <i class='fa fa-share'></i> {$lang['gl_shareratio']}<span class='label label-info'>" . member_ratio($CURUSER['uploaded'], $INSTALLER09['ratio_free'] ? "0" : $CURUSER['downloaded']) . "</span><br />";
    if ($INSTALLER09['ratio_free']) {
        $StatusBar.= "
        <i class='fa fa-arrow-up'></i> {$lang['gl_uploaded']} <span class='label label-success'>$upped</span>";
    } else {
        $StatusBar.= "
        <i class='fa fa-upload'></i> {$lang['gl_uploaded']} <span class='label label-success'>$upped</span>
	<i class='fa fa-download'></i> {$lang['gl_downloaded']}<span class='label label-success'>$downed</span><br />";
    }
    $StatusBar.= "<i class='fa fa-arrow-up'></i> {$lang['gl_seed_torrents']} <span class='label label-success'>{$seed['yes']}</span>
		<i class='fa fa-arrow-down'></i> {$lang['gl_leech_torrents']} <span class='label label-success'>{$seed['no']}</span><br />
		<i class='fa fa-exchange'></i> {$lang['gl_connectable']} <span class='label label-success'>{$connectable}</span>";
    return $StatusBar;
}
?>

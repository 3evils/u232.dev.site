...
(function($) {
	...
	var iCarousel1 = function(el, slides, options) {
		var ic = this;
		...
		el.append('<div id="iCarousel1-preloader"><div></div></div>');
		var iCarousel1Preloader = $('#iCarousel1-preloader', el);
		var preloaderBar = $('div', iCarousel1Preloader);
		iCarousel1Preloader.css({
			top: ((ic.defs.height / 2) - (iCarousel1Preloader.height() / 2)) + 'px',
			left: ((ic.defs.width / 2) - (iCarousel1Preloader.width() / 2)) + 'px'
		});

		//Set Timer Element
		el.append('<div id="iCarousel1-timer"><div></div></div>');
		ic.iCarousel1Timer = $('#iCarousel1-timer', el);
		ic.iCarousel1Timer.hide();
		ic.barTimer = $('div', ic.iCarousel1Timer);
		...
		if (ic.defs.total > 1 && ic.defs.timer != "bar") {
			...
			var width = (diameter + (padding * 2) + (stroke * 2)),
				height = width,
				r = Raphael(ic.iCarousel1Timer[0], width, height),
				R = (diameter / 2),
				param = {
	...
	//Start the iCarousel1
	iCarousel1.prototype = {
		...
		init: function() {
			...
			ic.iCarousel1Timer.attr('title', ic.options.playLabel).addClass('paused').show();

			//Start the timer
			if (ic.options.autoPlay && ic.defs.total > 1) {
				ic.setTimer();
				ic.iCarousel1Timer.attr('title', ic.options.pauseLabel).removeClass('paused');
			}
		},
		...
		events: function() {
			...
			if (ic.options.keyboardNav) $(document).bind('keyup.iCarousel1', function(event) {
			...
			// Navigation buttons
			$('a#iCarousel1Prev', ic.el).click(function() {
				ic.goPrev();
			});
			$('a#iCarousel1Next', ic.el).click(function() {
				ic.goNext();
			});

			//Play/Pause action
			ic.iCarousel1Timer.click(function() {

				if (ic.iCarousel1Timer.hasClass('paused')) {
					ic.iCarousel1Timer.removeClass('paused').attr('title', ic.options.pauseLabel);
					ic.defs.pause = false;
					...
					ic.iCarousel1Timer.addClass('paused').attr('title', ic.options.playLabel);
					ic.defs.pause = true;
					clearInterval(ic.defs.interval);
					ic.defs.interval = null;
			...
			ic.el.bind('iCarousel1:pause', function() {
				ic.iCarousel1Timer.addClass('paused').attr('title', ic.options.playLabel);
				ic.defs.pause = true;
				clearInterval(ic.defs.interval);
				ic.defs.interval = null;
			...
			ic.el.bind('iCarousel1:play', function() {
				ic.iCarousel1Timer.removeClass('paused').attr('title', ic.options.pauseLabel);
				ic.defs.pause = false;
			...
			ic.el.bind('iCarousel1:goSlide', function(event, slide) {
				if (ic.defs.slide != slide) ic.goFar(slide);
			});

			//Bind the next action
			ic.el.bind('iCarousel1:next', function() {
				ic.goNext();
			});

			//Bind the previous action
			ic.el.bind('iCarousel1:previous', function() {
				ic.goPrev();
			});
		...
		setButtons: function() {
			this.el.append('<a class="iCarousel1Nav" id="iCarousel1Prev" title="' + this.options.previousLabel + '">' + this.options.previousLabel + '</a><a class="iCarousel1Nav" id="iCarousel1Next" title="' + this.options.nextLabel + '">' + this.options.nextLabel + '</a>');
		},
	...
	// Begin the iCarousel1 plugin
	$.fn.iCarousel1 = function(options) {
		...
		$(this).each(function() {
			...
			new iCarousel1(el, slides, options);
		});

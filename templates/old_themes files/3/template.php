...
if (!isset($_NO_COMPRESS)) if (!ob_start('ob_gzhandler')) ob_start();
	...
	$inbox = ($unread1 == 1 ? "$unread1&nbsp;" : "$unread1&nbsp;");
$htmlout .='
<!DOCTYPE html>
	...
	<head>
		...
		<!-- Template CSS-->
		<link rel="stylesheet" href="'. $INSTALLER09['baseurl'] . '/templates/3/css/bootstrap-cerulean.css" type="text/css">
		<link rel="stylesheet" href="'. $INSTALLER09['baseurl'] . '/templates/3/3.css" />
		<link rel="stylesheet" href="'. $INSTALLER09['baseurl'] . '/templates/3/css/font-awesome-updated.css" />
		<!-- Captcha script-->
		<script type="text/javascript" src="'. $INSTALLER09['baseurl'] . '/scripts/captcha.js"></script>
		<link rel="stylesheet" href="'. $INSTALLER09['baseurl'] . '/css/global_media.css" type="text/css" />
        <link rel="stylesheet" href="css/global_media.css" type="text/css" />
		<script type="text/javascript" src="'. $INSTALLER09['baseurl'] . '/templates/3/js/icarousel-iseeyoucopy.js"></script>
		<script type="text/javascript" src="'. $INSTALLER09['baseurl'] . '/templates/3/js/raphael-min-iseeyoucopy.js"></script>
		<!-- Forum CSS-->
		<link rel="stylesheet" href="'. $INSTALLER09['baseurl'] . '/templates/3/css/forum.css" />
		<!-- global javascript-->
		<script src="'. $INSTALLER09['baseurl'] . '/scripts/jquery-1.11.1.js"></script>
		<!--[if lt IE 7]>      <html class="no-js lt-ie9 lt-ie8 lt-ie7"> <![endif]-->
		<!--[if IE 7]>         <html class="no-js lt-ie9 lt-ie8"> <![endif]-->
		<!--[if IE 8]>         <html class="no-js lt-ie9"> <![endif]-->
		<!--[if gt IE 8]> <![endif]-->
		<!--[if lt IE 9]><meta http-equiv="X-UA-Compatible" content="IE=9"><![endif]-->
		<!-- <script src="'. $INSTALLER09['baseurl'] . '/scripts/html5shiv.js"  async></script>  -->
		<script src="'. $INSTALLER09['baseurl'] . '/scripts/respond.min.js"  async></script> <!-- used for IE8 and below-->
	...
	<body>
		...
		if ($CURUSER) {
		$htmlout.="<nav class='navbar-default role='navigation'>
				...
				<div id='navbar' class='navbar-collapse collapse'>
					<ul class='nav navbar-nav'>
						<div class='btn-group' role='group' aria-label='...'>
							<div class='btn-group'>
							<button type='button' href='#' class='btn btn-default navbar-btn dropdown-toggle' data-toggle='dropdown' aria-haspopup='true' aria-expanded='false'>{$lang['gl_general']}<span class='caret'></span></button>
							...
							<div class='btn-group'>
							<button type='button' href='#' class='btn btn-default navbar-btn dropdown-toggle' data-toggle='dropdown' aria-haspopup='true' aria-expanded='false'>{$lang['gl_torrent']}<span class='caret'></span></button>
								<ul class='dropdown-menu'>
							...
							<div class='btn-group'>
							<button type='button' href='#' class='btn btn-default navbar-btn dropdown-toggle' data-toggle='dropdown' aria-haspopup='true' aria-expanded='false'>{$lang['gl_games']}<span class='caret'></span></button>
								<ul class='dropdown-menu'>
							...
							<a class='btn btn-success navbar-btn' href='" . $INSTALLER09['baseurl'] . "/donate.php'>{$lang['gl_donate']}</a>
							<a class='btn navbar-btn btn-info' href='" . $INSTALLER09['baseurl'] . "/forums.php'>{$lang['gl_forums']}</a>
							<div class='btn-group'>
							<button type='button' href='#' class='btn btn-default navbar-btn dropdown-toggle' data-toggle='dropdown' aria-expanded='false'>Staff Tools<span class='caret'></span></button>
								<ul class='dropdown-menu'>
									<!-- dropdown menu links -->
					...
					<ul class='nav navbar-nav navbar-right'>
						<div class='btn-group' role='group' aria-label='...'>
							<div class='btn-group'>
							<button type='button' href='#' class='btn btn-default navbar-btn dropdown-toggle' data-toggle='dropdown' aria-haspopup='true' aria-expanded='false'>Pers Tools<span class='caret'></span></button>
								<ul class='dropdown-menu'>
									<!-- dropdown menu links -->
							...
							<div class='btn-group'>
								" . (isset($CURUSER) && $CURUSER['class'] > UC_STAFF ? "<button type='button' href='#modal' data-toggle='modal' class='btn btn-default navbar-btn' data-target='#staff'><i class='fa fa-user-md' aria-hidden='true'></i></button>" : "") . "
								<button type='button' href='#modal' data-toggle='modal' class='btn btn-default navbar-btn' data-target='#messages'>
									<i class='fa fa-envelope' aria-hidden='true'></i>
									<i class='badge'><a href='pm_system.php'>$inbox</a></i>
								</button>
								<button type='button' class='btn btn-default navbar-btn' href='#collapseStatusBar' data-toggle='collapse' aria-expanded='false' aria-controls='collapseStatusBar'>
									<i class='fa fa-user-circle' aria-hidden='true'></i>
								</button>
								<button type='button' class='btn btn-default navbar-btn' href='#collapseAlerts' data-toggle='collapse' role='button' aria-expanded='false' aria-controls='collapseAlerts'>
									<i class='fa fa-bell-o' aria-hidden='true'></i>
								</button>
								<a class='btn navbar-btn btn-danger' href='" . $INSTALLER09['baseurl'] . "/logout.php?hash_please={$salty}'><i class='fa fa-sign-out' aria-hidden='true'></a></i>
							</div>
						</div>
					</ul>
				</div><!--/.nav-collapse -->
...
if ($CURUSER) {
$htmlout .="<div class='collapse' id='collapseStatusBar'>
				<div class='well'>" . StatusBar() . "</div>
			</div>
...
$htmlout.= "<div class='container-fluid'><a href='" . $INSTALLER09['baseurl'] . "/index.php'><img src='templates/3/pic/banner.png' class='img-responsive'></a></div>";
if ($CURUSER) {
$htmlout.= '
	...
	<div class="modal-dialog modal-lg" role="document">
		<div class="modal-content container-fluid">
			...
			<div class='container col-xs-12'>
				<div class='row'>
					<div class='col-md-3'>
					<li class='list-group-item'><b>User</b></li>
					<ul class='list-group'>";
					...
					$htmlout .= "
					<div class='col-md-3'>
					<li class='list-group-item'><b>Stats</b></li>
					<ul class='list-group'>";
					foreach ($mysql_data as $key => $value){
			...
			<div class="table-responsive">
				<table class="table">
					...
					<tbody>
						<tr>
							<td>' . $sender . '</td>
							<td>' . ($quickmessage['subject'] !== '' ? htmlsafechars($quickmessage['subject']) : 'no subject' ) . '</td>
							<td>' . htmlsafechars($quickmessage['msg']) . '</td>
							<td>' . get_date($quickmessage['added'], '') . '</td>
						</tr>
					</tbody>
				</table>
			</div>';
...
<div class='panel panel-default'>
  ...
  if ($CURUSER) {
    ...
    $htmlfoot.='
		...
		<script src="/templates/3/js/bootstrap.js"></script>
        </body></html>';
    return $htmlfoot;
}

...
<link rel='stylesheet' href='/templates/3/css/app.css' type='text/css' />
<style type='text/css'>
...
if ($CURUSER['chatpost'] == 0 || $CURUSER['chatpost'] > 1) {
    $HTMLOUT.= "<div class='error' align='center'><br /><font color='red'>{$lang['shoutbox_banned']}</font>  (<a href=\"./rules.php\" target=\"_blank\"><font color='red'>{$lang['shoutbox_banned_why']}</font></a>)<br /><br /></div>";
    echo $HTMLOUT;
    exit;
}
//=End
...
if (count($shouts) > 0) {
    $HTMLOUT.= "<table class='table small text-left' style='clear:both; border-collapse:collapse; width:100%;'>\n";
    ...
    if ($shouts) {
        ...
        foreach ($shouts as $arr) {
            ...
            $edit = ($CURUSER['class'] >= UC_STAFF || ($arr['userid'] == $CURUSER['id']) && ($CURUSER['class'] >= UC_POWER_USER && $CURUSER['class'] <= UC_STAFF) ? "<a href='{$INSTALLER09['baseurl']}/shoutbox.php?edit=" . (int)$arr['id'] . "&amp;user=" . (int)$arr['userid'] . "'><i class='fas fa-pencil-alt' style='color:#ffae00'></i></a> " : "");
            $del = ($CURUSER['class'] >= UC_STAFF ? "<a href='./shoutbox.php?del=" . (int)$arr['id'] . "'><i class='fas fa-trash-alt'></i></a> " : "");
            $delall = ($CURUSER['class'] == UC_MAX ? "<a href='./shoutbox.php?delall' onclick=\"confirm_delete(); return false;\"><i class='fas fa-times' style='color:red'></i></a> " : "");
            //$delall
            $pm = ($CURUSER['id'] != $arr['userid'] ? "<span class='date' style=\"color:$dtcolor\"><a target='_blank' href='./pm_system.php?action=send_message&amp;receiver=" . (int)$arr['userid'] . "'><i class='far fa-envelope'></i></a></span>\n" : "");
            $date = get_date($arr["date"], 0, 1);
            $reply = ($CURUSER['id'] != $arr['userid'] ? "<a href=\"javascript:window.top.SmileIT('[b][i]&nbsp;[color=#" . get_user_class_color($arr['class']) . "]" . ($arr['perms'] & bt_options::PERMS_STEALTH ? "UnKnown" : htmlsafechars($arr['username'])) . "[/color]&nbsp;-[/i][/b]','shbox','shbox_text')\"><i class='fas fa-reply' style='color:#3adb76'></i></a>" : "");
            $user_stuff = $arr;
            ...
            $HTMLOUT.= "<tr style='background-color:$bg;'>
				<td class='padding-0'>
				<!--<a type='button' data-toggle='clock'><i class='far fa-clock'></i></a><div class='dropdown-pane' data-position='right' data-alignment='center' id='clock' data-dropdown data-auto-focus='true'>". $date ."</div>-->
<span data-tooltip class='right' tabindex='1' title='". htmlsafechars($date)."'><i class='far fa-clock'></i></span>$del$edit$pm$reply$private
					" . format_username($user_stuff, true) . "
					<span class='border-box' style='color:$fontcolor;'><p class='display-inline'>" . format_comment($arr["text"]) . "</p></span>
